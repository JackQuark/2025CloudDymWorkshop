...
def main():
    ...
    with ds_case.open_ncdataset("thermo", step=sel_tstep, preprocess=partial_func) as ds:
        ...
        qi = ds.qi.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1]))
        qcloud = qc + qi
    ...
    levels = np.linspace(qcloud.min(), qcloud.max(), 5)
    axs[0].contour(_X, _m_zc, qcloud[_t, :, :], levels=levels, alpha=0.7)
