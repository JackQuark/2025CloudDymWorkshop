...
def main():
    ...
    # res = sel_bresenham_line(48, 20, 48, 80)
    res = sel_bresenham_line(57, 30, 35, 75)

    # res = sel_bresenham_line(*loc_to_idx(121.45, 24.785, isel_lon, isel_lat),
    #                          *loc_to_idx(121.62, 24.967, isel_lon, isel_lat))
    ...
    _topo = ds.topo.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1])).astype(int)
    ...
    _topo_da = xr.DataArray(m_zc[_topo]/1000, dims=["dim_0"])
    with ds_case.open_ncdataset("thermo", step=sel_tstep, preprocess=partial_func) as ds:
        ...
        qv: xr.DataArray = ds.qv.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1])).where(ds.lev >= _topo_da)
        qc: xr.DataArray = ds.qc.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1])).where(ds.lev >= _topo_da)
        qr: xr.DataArray = ds.qr.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1])).where(ds.lev >= _topo_da)
        qi: xr.DataArray = ds.qi.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1])).where(ds.lev >= _topo_da)
        thv: xr.DataArray = calc_th_v(th, qv, qc + qr)
        thv_bar  = thv.mean(dim='dim_0', skipna=True)
        thv_anom = thv - thv_bar
    del ds
    _m_zc = m_zc[isel_lev]
    # _p_zc = p_zc[isel_lev]
    ...
    norm = mplc.Normalize(vmin=-3, vmax=3)
    CSpm = axs[0].pcolormesh(_X, _m_zc, thv_anom.isel(time=_t), cmap=cmap, norm=norm, alpha=0.6, zorder=0)
    cbar = fig.colorbar(CSpm, ax=axs, shrink=0.75)
    cbar.set_label(r"$\theta_v$ anomaly [K]")
    ...
    axs[0].contour(_X, _m_zc, qc.isel(time=_t) + qi.isel(time=_t), alpha=0.5, levels=[0.00001])

    print("fig. init complete")
    return 
    def update_plot(i):
        print(f"{i}", end='\r')
        axs[0].collections[-1].remove()
        axs[0].set_title(f"{sel_case_name}, @ LST {f'{i//6+10:02d}:{i%6*10:02d}'}\ncross-section MSE / wind", loc='left')
        L_sprec.set_ydata(sprec.isel(time=i))
        Q.set_UVC(V_h[i, _zskip, _xskip], w[i, _zskip, _xskip])
        CSpm.set_array(thv_anom.isel(time=i))
        axs[0].contour(_X, _m_zc, qc.isel(time=i) + qi.isel(time=i), alpha=0.5, levels=[0.0002])
