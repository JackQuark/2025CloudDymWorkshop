...
def main():
    ...
    with ds_case.open_ncdataset("thermo", step=sel_tstep, preprocess=partial_func) as ds:
        ...
        # qc = ds.qc.isel(lon=xr.DataArray(res[:, 0]), lat=xr.DataArray(res[:, 1]))
    ...
    Q = axs[0].quiver(_X[_xskip], _m_zc[_zskip], 
                      v.isel(time=_t, lev=_zskip, dim_0=_xskip), 
    ...
    levels = np.linspace(320000, 360000, 21)
    norm   = plt.Normalize(320000, 360000)
    CS = axs[0].pcolormesh(_X, _m_zc, MSE.isel(time=_t), cmap='turbo', norm=norm, alpha=0.6, zorder=0)
    cbar = fig.colorbar(CS, ax=axs, shrink=0.75)
    cbar.set_ticks(levels[::2])
    cbar.set_ticklabels([f"{int(t/1000)}" for t in cbar.get_ticks()])
    cbar.set_label("MSE [KJ/kg]")
    cbar.ax.yaxis.set_label_position('left')
    # axs[0].contour(_X, m_zc, qc[_t, :, :], alpha=0.5)
    ...
    axs[0].set_xticks([28], labels=["Wu-Lai"])
    axs[1].axvline(28, c='gray', ls='--')
    ...
    def update_plot(i):
        ...
        Q.set_UVC(v[i, _zskip, _xskip], w[i, _zskip, _xskip])
        CS.set_array(MSE.isel(time=i))
