...
def main():
    ...
    plot_sprec(np.sum(hourly_sprec, axis=0), lat, lon, topo=topo, ax=ax)
    ax.plot(central_lon, central_lat, '*', c='cyan', ms=10, zorder=20, label=f"121.5682E, 24.88368N")
    ax.set_facecolor('whitesmoke')
    _plot_boundaries(ax, boundaries="city")
    ax.set_aspect('equal')
    ax.set_xlabel("Longitude")
    ax.set_ylabel("Latitude")
    ax.legend(loc='lower right', bbox_to_anchor=(1, 1.005))
    ax.set_title(f"{sel_case},\ndaily total sprec", loc='left')
    return 
